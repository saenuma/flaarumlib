...
func (cl *Client) CreateOrUpdateTable(stmt string) error {
	...
	tableStruct, err := ParseTableStructureStmt(stmt)
	if err != nil {
		return err
	}
	if !slices.Contains(tables, tableStruct.TableName) {
		// table doesn't exist
...
func (cl *Client) GetCurrentTableStructureParsed(tableName string) (TableStruct, error) {
